...
class BinanceSocketManager(threading.Thread):
    ...
    def create_ws_connection(self):
        ...
        if localproxy is not None:
            proxy_type = 'http'
            http_proxy_host = localproxy.split(':')[0]
            http_proxy_port = localproxy.split(':')[1]
            self.ws = create_connection(self.stream_url, http_proxy_host=http_proxy_host, http_proxy_port=http_proxy_port, proxy_type=proxy_type)
        else:
    ...
    def run(self):
        try:
            ...
            print('read data run excetion:', e)
