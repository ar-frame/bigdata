...
class Shipan:
    ...
    def initResource(self):
        tradeVariety = self.tradeVariety
        ...
        self.db_mongo_store = MongoDB(mongo_store.get('DB'), 'orders_' + func.formatStoreName(self.tradeVariety) + '_' + self.account_name, mongo_store.get('DB_HOST'), mongo_store.get('DB_USER'), mongo_store.get('DB_PASS'))

        self.paint = Paint(self.account_name)
        self.sim = self.paint.sim
        self.cha = self.paint.cha

        self.trade = Trade(self.account_name)
        self.threads = []
        self.orders = []
    ...
    def stepOrder(self):
        ...
        try:
            while True:
                ...
                print('step order loop account: %s,pid: %d' % (self.account_name, os.getpid()))
            ...
            cfg.write_log('ShipanStepOrderException', e)
            cfg.write_log('ShipanStepOrderException', traceback.print_exc())
            cfg.write_log('ShipanStepOrderException', 'traceback.format_exc():\n%s' % traceback.format_exc())
    ...
    def getOrderFromStore(self):
        return list(self.db_mongo_store.query_all(con={"complete": {"$eq": '0'}})) + list(
            self.db_mongo_store.query_all(con={"complete": {"$eq": 0}}))

    def autoStart(self):

        store_orders = self.getOrderFromStore()
    ...
    def start(self):
        # sp = Shipan(account_name)
        # sp.addOrder(optype = 'buy', currency = 2500, price = 13000, timedate = None, code = None, ispc = False)

        # store_orders = sp.getOrderFromStore()
        # print(store_orders)
        # exit()
        while True:
            tools.checkMainProcessStatusAndExit()
